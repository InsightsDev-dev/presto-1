 ...
 * @Date 13-Apr-2015
 */
public class CustomizedPredicatePushDown extends PlanOptimizer {
	// private static final Logger log = Logger
	// .get(CustomizedPredicatePushDown.class);
	private final Metadata metadata;
	private final SqlParser sqlParser;
	private final SplitManager splitManager;
	private final boolean experimentalSyntaxEnabled;

	public CustomizedPredicatePushDown(Metadata metadata, SqlParser sqlParser,
			SplitManager splitManager, boolean experimentalSyntaxEnabled) {
	...
	public PlanNode optimize(PlanNode plan, Session session,
		...
		throw new UnsupportedOperationException(
				"Method not implemented.instead pass CustomizedPredicatePushDownContext");
	}
	...
	public PlanNode optimize(
		...
		return PlanRewriter.rewriteWith(new Rewriter(symbolAllocator,
				idAllocator, metadata, sqlParser, splitManager, session,
				experimentalSyntaxEnabled, customizedPredicatePushDownContext),
				plan, PredicatePushDownContext.TRUE_LITERAL());
	}

	private static class Rewriter extends
			PlanNodeRewriter<PredicatePushDownContext> {
		...
		private Rewriter(
				...
				CustomizedPredicatePushDownContext customizedPredicatePushDownContext) {
			this.symbolAllocator = checkNotNull(symbolAllocator,
		...
		@Override
		public PlanNode rewriteNode(PlanNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			PlanNode rewrittenNode = planRewriter.defaultRewrite(node,
					PredicatePushDownContext.TRUE_LITERAL());
			if (!inheritedPredicate.getExpression().equals(
					BooleanLiteral.TRUE_LITERAL)) {
				// Drop in a FilterNode b/c we cannot push our predicate down
				// any further
				rewrittenNode = new FilterNode(idAllocator.getNextId(),
						rewrittenNode, inheritedPredicate.getExpression());
			}
			return rewrittenNode;
		}

		// Aggregate information can be push down
		@Override
		public PlanNode rewriteProject(ProjectNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {

			// Expression inlinedPredicate = ExpressionTreeRewriter.rewriteWith(
			// new ExpressionSymbolInliner(node.getAssignments()),
			// inheritedPredicate.getExpression());
			// inheritedPredicate.setExpression(inlinedPredicate);
			...
			Expression inlinedPredicate = ExpressionTreeRewriter.rewriteWith(
					...
					inheritedPredicate.getExpression());
			inheritedPredicate.setExpression(inlinedPredicate);
			HashMap<Symbol, Expression> hashMap = new HashMap<Symbol, Expression>();
			hashMap.putAll(node.getAssignments());
			ProjectNode projectNode = new ProjectNode(node.getId(),
					node.getSource(), hashMap);
			ProjectNode projectNode2 = planRewriter.defaultRewrite(projectNode,
					inheritedPredicate);
			HashMap<Symbol, Expression> hashMap2 = new HashMap<Symbol, Expression>();
			for (Entry<Symbol, Expression> entry : projectNode2
			...
			return new ProjectNode(projectNode2.getId(),
					projectNode2.getSource(), hashMap2);
		}

		// This is used in case of distinct in aggregates so we can push down
		// from here.

		@Override
		public PlanNode rewriteMarkDistinct(MarkDistinctNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			checkState(
					!DependencyExtractor.extractUnique(
							inheritedPredicate.getExpression()).contains(
		...
		// Aggregate information can be push down
		@Override
		public PlanNode rewriteSort(SortNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			return planRewriter.defaultRewrite(node, inheritedPredicate);
		}

		// We don't know About Aggregate information can be push down

		@Override
		public PlanNode rewriteUnion(UnionNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			boolean modified = false;
			ImmutableList.Builder<PlanNode> builder = ImmutableList.builder();
			for (int i = 0; i < node.getSources().size(); i++) {
				Expression sourcePredicate = ExpressionTreeRewriter
						.rewriteWith(
								new ExpressionSymbolInliner(node
										.sourceSymbolMap(i)),
								inheritedPredicate.getExpression());
				PlanNode source = node.getSources().get(i);
				PlanNode rewrittenSource = planRewriter.rewrite(source,
						new PredicatePushDownContext(sourcePredicate));
		...
		// We know About Aggregate information can be push down here

		@Override
		public PlanNode rewriteFilter(FilterNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			inheritedPredicate.setExpression(combineConjuncts(
					node.getPredicate(), inheritedPredicate.getExpression()));
			return planRewriter.rewrite(node.getSource(), inheritedPredicate);
		}

		// We know About Aggregate information can not be push down here

		@Override
		public PlanNode rewriteJoin(JoinNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			boolean isCrossJoin = (node.getType() == JoinNode.Type.CROSS);

			// See if we can rewrite outer joins in terms of a plain inner join
			node = tryNormalizeToInnerJoin(node,
					inheritedPredicate.getExpression());
			...
			case INNER:
				InnerJoinPushDownResult innerJoinPushDownResult = processInnerJoin(
						inheritedPredicate.getExpression(),
						leftEffectivePredicate, rightEffectivePredicate,
						joinPredicate, node.getLeft().getOutputSymbols());
				...
				break;
			case LEFT:
				OuterJoinPushDownResult leftOuterJoinPushDownResult = processOuterJoin(
						inheritedPredicate.getExpression(),
						leftEffectivePredicate, rightEffectivePredicate,
						joinPredicate, node.getLeft().getOutputSymbols());
				...
				postJoinPredicate = leftOuterJoinPushDownResult
						.getPostJoinPredicate();
				newJoinPredicate = joinPredicate; // Use the same as the
													// original
				break;
			case RIGHT:
				OuterJoinPushDownResult rightOuterJoinPushDownResult = processOuterJoin(
						inheritedPredicate.getExpression(),
						rightEffectivePredicate, leftEffectivePredicate,
						joinPredicate, node.getRight().getOutputSymbols());
			...
			PlanNode leftSource = planRewriter.rewrite(node.getLeft(),
					new PredicatePushDownContext(leftPredicate));
			PlanNode rightSource = planRewriter.rewrite(node.getRight(),
					new PredicatePushDownContext(rightPredicate));

			PlanNode output = node;
			if (leftSource != node.getLeft() || rightSource != node.getRight()
					|| !newJoinPredicate.equals(joinPredicate) || isCrossJoin) {
				List<JoinNode.EquiJoinClause> criteria = node.getCriteria();

				// Rewrite criteria and add projections if there is a new join
				// predicate
				...
				if (!newJoinPredicate.equals(joinPredicate) || isCrossJoin) {
					...
					ImmutableMap.Builder<Symbol, Expression> leftProjections = ImmutableMap
							.builder();
					leftProjections.putAll(IterableTransformer
							.<Symbol> on(node.getLeft().getOutputSymbols())
							.toMap(symbolToQualifiedNameReference()).map());
					ImmutableMap.Builder<Symbol, Expression> rightProjections = ImmutableMap
							.builder();
					rightProjections.putAll(IterableTransformer
							.<Symbol> on(node.getRight().getOutputSymbols())
							.toMap(symbolToQualifiedNameReference()).map());

					// HACK! we don't support cross joins right now, so put in a
					// simple fake join predicate instead if all of the join
					// clauses got simplified out
					// TODO: remove this code when cross join support is added
					Iterable<Expression> simplifiedJoinConjuncts = transform(
							extractConjuncts(newJoinPredicate),
							simplifyExpressions());
					simplifiedJoinConjuncts = filter(
							simplifiedJoinConjuncts,
							not(Predicates
									.<Expression> equalTo(BooleanLiteral.TRUE_LITERAL)));
					if (Iterables.isEmpty(simplifiedJoinConjuncts)) {
						simplifiedJoinConjuncts = ImmutableList
								.<Expression> of(new ComparisonExpression(
										ComparisonExpression.Type.EQUAL,
										new LongLiteral("0"), new LongLiteral(
												"0")));
					}
					...
					for (Expression conjunct : simplifiedJoinConjuncts) {
						...
						boolean alignedComparison = Iterables.all(
								DependencyExtractor.extractUnique(equality
										.getLeft()), in(node.getLeft()
										.getOutputSymbols()));
						Expression leftExpression = (alignedComparison) ? equality
								.getLeft() : equality.getRight();
						Expression rightExpression = (alignedComparison) ? equality
								.getRight() : equality.getLeft();
						...
						builder.add(new JoinNode.EquiJoinClause(leftSymbol,
								rightSymbol));
					}
				...
				output = new JoinNode(node.getId(), node.getType(), leftSource,
						rightSource, criteria, node.getComparisons());
			}
		...
		private OuterJoinPushDownResult processOuterJoin(
			...
			checkArgument(Iterables.all(
					DependencyExtractor.extractUnique(outerEffectivePredicate),
					in(outerSymbols)),
					"outerEffectivePredicate must only contain symbols from outerSymbols");
			checkArgument(Iterables.all(
					DependencyExtractor.extractUnique(innerEffectivePredicate),
					not(in(outerSymbols))),
			...
			Expression outerOnlyInheritedEqualities = combineConjuncts(equalityPartition
					.getScopeEqualities());
			...
			for (Expression conjunct : EqualityInference
				...
				Expression outerRewritten = outerInference.rewriteExpression(
						conjunct, in(outerSymbols));
			...
			// See if we can push down any outer or join predicates to the inner
			// side
			for (Expression conjunct : EqualityInference
					.nonInferrableConjuncts(and(outerEffectivePredicate,
							joinPredicate))) {
			...
			outerPushdownConjuncts.addAll(equalityPartition
					.getScopeEqualities());
			postJoinConjuncts.addAll(equalityPartition
			...
			innerPushdownConjuncts
					.addAll(potentialNullSymbolInferenceWithoutInnerInferred
							.generateEqualitiesPartitionedBy(
									not(in(outerSymbols))).getScopeEqualities());
		...
		private static class OuterJoinPushDownResult {
			...
			private OuterJoinPushDownResult(Expression outerJoinPredicate,
					Expression innerJoinPredicate, Expression postJoinPredicate) {
		...
		private InnerJoinPushDownResult processInnerJoin(
			...
			checkArgument(Iterables.all(
					DependencyExtractor.extractUnique(leftEffectivePredicate),
					in(leftSymbols)),
					"leftEffectivePredicate must only contain symbols from leftSymbols");
			checkArgument(Iterables.all(
					DependencyExtractor.extractUnique(rightEffectivePredicate),
					not(in(leftSymbols))),
			...
			for (Expression conjunct : EqualityInference
				...
				Expression leftRewritten = allInference.rewriteExpression(
						conjunct, in(leftSymbols));
				if (leftRewritten != null) {
					leftPushDownConjuncts.add(leftRewritten);
				}

				Expression rightRewritten = allInference.rewriteExpression(
						conjunct, not(in(leftSymbols)));
			...
			rightPushDownConjuncts.addAll(allInferenceWithoutRightInferred
					...
					.getScopeEqualities());
			joinConjuncts.addAll(allInference.generateEqualitiesPartitionedBy(
					in(leftSymbols)).getScopeStraddlingEqualities()); // scope
																		// straddling
																		// equalities
																		// get
																		// dropped
			...
			List<Expression> postJoinConjuncts = ImmutableList
					.copyOf(filter(joinConjunctsList,
							not(joinEqualityExpression(leftSymbols))));
			joinConjunctsList = ImmutableList.copyOf(filter(joinConjunctsList,
					joinEqualityExpression(leftSymbols)));
		...
		private JoinNode tryNormalizeToInnerJoin(JoinNode node,
				Expression inheritedPredicate) {
			Preconditions.checkArgument(EnumSet.of(INNER, RIGHT, LEFT, CROSS)
					.contains(node.getType()), "Unsupported join type: %s",
					node.getType());
			...
			if (node.getType() == JoinNode.Type.INNER
					...
					&& !canConvertOuterToInner(node.getRight()
							.getOutputSymbols(), inheritedPredicate)
					|| node.getType() == JoinNode.Type.RIGHT
					&& !canConvertOuterToInner(node.getLeft()
							.getOutputSymbols(), inheritedPredicate)) {
				return node;
			}
			return new JoinNode(node.getId(), JoinNode.Type.INNER,
			...
			return new Function<Expression, Expression>() {
				...
				public Expression apply(Expression expression) {
					IdentityHashMap<Expression, Type> expressionTypes = getExpressionTypes(
							session, metadata, sqlParser,
							symbolAllocator.getTypes(), expression);
		...
		private Object nullInputEvaluator(final Collection<Symbol> nullSymbols,
			...
			IdentityHashMap<Expression, Type> expressionTypes = getExpressionTypes(
					...
					expression);
			return ExpressionInterpreter.expressionOptimizer(expression,
					metadata, session, expressionTypes).optimize(
					new SymbolResolver() {
						@Override
						public Object getValue(Symbol symbol) {
							return nullSymbols.contains(symbol) ? null
									: new QualifiedNameReference(symbol
											.toQualifiedName());
						}
					});
		}
				...
				public boolean apply(Expression expression) {
					...
					if (isDeterministic(expression)
						...
						if (comparison.getType() == ComparisonExpression.Type.EQUAL) {
							...
							return (Iterables.all(symbols1, in(leftSymbols)) && Iterables
									.all(symbols2, not(in(leftSymbols))))
									|| (Iterables
											.all(symbols2, in(leftSymbols)) && Iterables
											.all(symbols1, not(in(leftSymbols))));
						}
					}
					return false;
				}
		...
		// we know aggregate information can not be push down here
		@Override
		public PlanNode rewriteSemiJoin(SemiJoinNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			...
			EqualityInference joinInference = createEqualityInference(
					inheritedPredicate.getExpression(),
					sourceEffectivePredicate, joinPredicate);
			for (Expression conjunct : Iterables.concat(
					EqualityInference.nonInferrableConjuncts(inheritedPredicate
							.getExpression()), EqualityInference
							.nonInferrableConjuncts(sourceEffectivePredicate))) {
			...
			EqualityInference.EqualityPartition joinInferenceEqualityPartition = joinInference
					.generateEqualitiesPartitionedBy(equalTo(node
							.getFilteringSourceJoinSymbol()));
			filteringSourceConjuncts.addAll(ImmutableList.copyOf(transform(
					joinInferenceEqualityPartition.getScopeEqualities(),
					expressionOrNullSymbols(equalTo(node
							.getFilteringSourceJoinSymbol())))));
			...
			EqualityInference inheritedInference = createEqualityInference(inheritedPredicate
					.getExpression());
			for (Expression conjunct : EqualityInference
					.nonInferrableConjuncts(inheritedPredicate.getExpression())) {
			...
			EqualityInference.EqualityPartition equalityPartition = inheritedInference
					.generateEqualitiesPartitionedBy(in(node.getSource()
							.getOutputSymbols()));
			...
			PlanNode rewrittenSource = planRewriter.rewrite(node.getSource(),
					new PredicatePushDownContext(
							combineConjuncts(sourceConjuncts)));
			PlanNode rewrittenFilteringSource = planRewriter.rewrite(node
					.getFilteringSource(), new PredicatePushDownContext(
		...
		// We will actually push down information from here.Any aggregate
		// information passed from above shall be merged here.
		@Override
		public PlanNode rewriteAggregation(AggregationNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			try {
				// if (customizedPredicatePushDownContext
				// .getSymbolToColumnAggregationMap().get(node) != null) {
				// symbolToColumnAggregation =
				// customizedPredicatePushDownContext
				// .getSymbolToColumnAggregationMap().get(node);
				// } else {
			...
			// Strip out non-deterministic conjuncts
			postAggregationConjuncts.addAll(ImmutableList.copyOf(filter(
					extractConjuncts(inheritedPredicate.getExpression()),
					not(deterministic()))));
			inheritedPredicate = new PredicatePushDownContext(
					stripNonDeterministicConjuncts(inheritedPredicate
							.getExpression()));
			...
			postAggregationConjuncts.addAll(equalityPartition
					.getScopeComplementEqualities());
			postAggregationConjuncts.addAll(equalityPartition
					.getScopeStraddlingEqualities());

			PredicatePushDownContext predicatePushDownContext = new PredicatePushDownContext(
			...
			if (predicatePushDownContext.getAggregations() != null
					...
					&& (predicatePushDownContext.getFunctionMap() != node
							.getFunctions()
							|| predicatePushDownContext.getAggregations() != node
									.getAggregations() || predicatePushDownContext
							.getGroupBy() != node.getGroupBy())) {
				...
				output = new AggregationNode(node.getId(), rewrittenSource,
						...
						node.getFunctions(), node.getMasks(), node.getStep(),
						node.getSampleWeight(), node.getConfidence());
			}
			// if (symbolToColumnAggregation != null) {
			// customizedPredicatePushDownContext
			// .getSymbolToColumnAggregationMap().put(output,
			// symbolToColumnAggregation);
			// }
		...
		// We don't know About Aggregate information can be push down
		@Override
		public PlanNode rewriteUnnest(UnnestNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			EqualityInference equalityInference = createEqualityInference(inheritedPredicate
					.getExpression());
			...
			// Strip out non-deterministic conjuncts
			postUnnestConjuncts.addAll(ImmutableList.copyOf(filter(
					extractConjuncts(inheritedPredicate.getExpression()),
					not(deterministic()))));
			inheritedPredicate = new PredicatePushDownContext(
					stripNonDeterministicConjuncts(inheritedPredicate
			...
			// Add the equality predicates back in
			EqualityInference.EqualityPartition equalityPartition = equalityInference
					.generateEqualitiesPartitionedBy(in(node
							.getReplicateSymbols()));
			pushdownConjuncts.addAll(equalityPartition.getScopeEqualities());
			postUnnestConjuncts.addAll(equalityPartition
					.getScopeComplementEqualities());
			postUnnestConjuncts.addAll(equalityPartition
					.getScopeStraddlingEqualities());
		...
		// We don't know About Aggregate information can be push down
		@Override
		public PlanNode rewriteSample(SampleNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			return planRewriter.defaultRewrite(node, inheritedPredicate);
		}

		// The actual aggregate information will be changed to relevant columns
		// and will be push down from here.
		@Override
		public PlanNode rewriteTableScan(TableScanNode node,
				PredicatePushDownContext inheritedPredicate,
				PlanRewriter<PredicatePushDownContext> planRewriter) {
			...
			if (predicatePushDownContext != null) {
				inheritedPredicate.setExpression(predicatePushDownContext
						.getExpression());
				inheritedPredicate.setAggregations(predicatePushDownContext
						.getAggregations());
				inheritedPredicate.setFunctionMap(predicatePushDownContext
						.getFunctionMap());
				inheritedPredicate.setGroupBy(predicatePushDownContext
						.getGroupBy());
			}

			DomainTranslator.ExtractionResult extractionResult = DomainTranslator
					.fromPredicate(metadata, session,
							inheritedPredicate.getExpression(),
							symbolAllocator.getTypes(), node.getAssignments());
			Expression extractionRemainingExpression = extractionResult
					.getRemainingExpression();
			TupleDomain<ColumnHandle> tupleDomain = extractionResult
					.getTupleDomain();

			if (node.getGeneratedPartitions().isPresent()) {
				// Add back in the TupleDomain that was used to generate the
				// previous set of Partitions if present
				// And just for kicks, throw in the domain summary too (as that
				// can only help prune down the ranges)
				// The domains should never widen between each pass.
				tupleDomain = tupleDomain.intersect(
						node.getGeneratedPartitions().get()
								.getTupleDomainInput()).intersect(
			...
			ProteumTupleDomain<ColumnHandle> proteumTupleDomain2 = new ProteumTupleDomain<ColumnHandle>(
					tupleDomain.getDomains(), extractionRemainingExpression);
			...
			PartitionResult matchingPartitions = splitManager.getPartitions(
					node.getTable(), Optional.of(proteumTupleDomain));
			List<Partition> partitions = matchingPartitions.getPartitions();
			TupleDomain<ColumnHandle> undeterminedTupleDomain = matchingPartitions
					.getUndeterminedTupleDomain();

			Expression unevaluatedDomainPredicate = DomainTranslator
					.toPredicate(undeterminedTupleDomain, ImmutableBiMap
							.copyOf(node.getAssignments()).inverse(),
							symbolAllocator.getTypes());
			// Construct the post scan predicate. Add the unevaluated
			// TupleDomain back first since those are generally cheaper to
			// evaluate than anything we can't extract
			Expression postScanPredicate = combineConjuncts(
					unevaluatedDomainPredicate, extractionRemainingExpression);
			...
			partitions = ImmutableList.copyOf(filter(
					partitions,
					not(shouldPrunePartition(postScanPredicate,
							node.getAssignments()))));
			GeneratedPartitions generatedPartitions = new GeneratedPartitions(
					tupleDomain, partitions);

			PlanNode output = node;
			if (!node.getGeneratedPartitions().equals(
					Optional.of(generatedPartitions))) {
				// Only overwrite the originalConstraint if it was previously
				// null
				Expression originalConstraint = node.getOriginalConstraint() == null ? inheritedPredicate
						.getExpression() : node.getOriginalConstraint();
				output = new TableScanNode(node.getId(), node.getTable(),
						node.getOutputSymbols(), node.getAssignments(),
						originalConstraint, Optional.of(generatedPartitions));
			}
			if (proteumTupleDomain2.isAggregatePushDownable()
					&& shouldPushDownAggregation) {
		...
		private Predicate<Partition> shouldPrunePartition(
				...
				final Map<Symbol, ColumnHandle> symbolToColumn) {
			final Map<ColumnHandle, Symbol> columnToSymbol = ImmutableBiMap
					.copyOf(symbolToColumn).inverse();
			final List<Expression> conjuncts = extractConjuncts(predicate);
			final IdentityHashMap<Expression, Type> expressionTypes = getExpressionTypes(
					session, metadata, sqlParser, symbolAllocator.getTypes(),
					predicate);
			...
			return new Predicate<Partition>() {
				...
				public boolean apply(Partition partition) {
					Map<ColumnHandle, Comparable<?>> columnFixedValueAssignments = partition
							.getTupleDomain().extractFixedValues();

					checkArgument(
							columnToSymbol.keySet().containsAll(
									columnFixedValueAssignments.keySet()),
							"assignments does not contain all required column handles");

					ImmutableMap.Builder<Symbol, Object> builder = ImmutableMap
							.builder();
					for (Map.Entry<ColumnHandle, Comparable<?>> entry : columnFixedValueAssignments
							.entrySet()) {
					...
					LookupSymbolResolver inputs = new LookupSymbolResolver(
							builder.build());

					// If any conjuncts evaluate to FALSE or null, then the
					// whole predicate will never be true and so the partition
					// should be pruned
					for (Expression expression : conjuncts) {
						ExpressionInterpreter optimizer = ExpressionInterpreter
								.expressionOptimizer(expression, metadata,
