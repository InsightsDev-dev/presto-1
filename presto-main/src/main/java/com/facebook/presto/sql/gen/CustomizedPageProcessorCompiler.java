        ...
        implements BodyCompiler<PageProcessor>
{
	...
	public void generateMethods(ClassDefinition classDefinition,
        ...
        generateFilterMethod(classDefinition, callSiteBinder, filter,offset);
        //generateFilterMethod(classDefinition, callSiteBinder, filter);
	}
	...
	private void generateFilterMethod(ClassDefinition classDefinition,
        ...
        MethodDefinition method = classDefinition.declareMethod(context,
                ...
                ImmutableList.<NamedParameterDefinition>builder()
                        .add(arg("session", ConnectorSession.class))
                        .add(arg("probeBlocks",type(List.class, com.facebook.presto.spi.block.Block.class)))
                        //buildBlocks
                        .add(arg("buildBlocks",type(List.class,type(List.class, com.facebook.presto.spi.block.Block.class))))
                        //.addAll(toBlockParameters(getInputChannels(filter)))
                        .add(arg("position1", int.class))
                        .add(arg("position2", int.class))
                        .add(arg("buildIndex",int.class))
                        .build());
...
////        	if(i==0)
////             channelField = classDefinition.declareField(a(PRIVATE), "block" , com.facebook.presto.spi.block.Block.class);
////        	else{
//                 channelField = classDefinition.declareField(a(PRIVATE), "block_" + i, com.facebook.presto.spi.block.Block.class);
//
//        	//}
...
//        }
        ...
        Variable positionVariable1 = context.getVariable("position1");
        Variable positionVariable2 = context.getVariable("position2");
        ...
        ByteCodeExpressionVisitor visitor = new ByteCodeExpressionVisitor(
                callSiteBinder,
                fieldReferenceCompiler(callSiteBinder, positionVariable1,positionVariable2, wasNullVariable,offset),
                metadata.getFunctionRegistry());
        ByteCodeNode body = filter.accept(visitor, context);
...
//                  .invoke("get", Object.class, com.facebook.presto.byteCode.expression.ByteCodeExpressions.constantInt(index))
...
//        }
	...
	private ByteCodeExpression getByteCodeExpresstion(CompilerContext context,int offset,int field){
		Variable first=context.getVariable("probeBlocks");
		Variable second=context.getVariable("buildBlocks");
        Variable buildIndex = context.getVariable("buildIndex");

		if(field<offset){
			return first.invoke("get", Object.class, com.facebook.presto.byteCode.expression.ByteCodeExpressions.constantInt(field));
		}else{
//			return second.invoke("get", Object.class, com.facebook.presto.byteCode.expression.ByteCodeExpressions.constantInt(field-offset))
//					.cast(type(List.class, com.facebook.presto.spi.block.Block.class)).invoke("get", Object.class, buildIndex);
	...
	private RowExpressionVisitor<CompilerContext, ByteCodeNode> fieldReferenceCompiler(
			final CallSiteBinder callSiteBinder, final Variable positionVariable1,
			final Variable positionVariable2, final Variable wasNullVariable,final int offset) {
        return new RowExpressionVisitor<CompilerContext, ByteCodeNode>()
        {
            @Override
            public ByteCodeNode visitInputReference(InputReferenceExpression node, CompilerContext context)
...
//             .invoke("get", Object.class, com.facebook.presto.byteCode.expression.ByteCodeExpressions.constantInt(field))
...
//             ;
                ...
                Block isNullCheck ;
                if(field<offset){
                isNullCheck= new Block(context)
                        .setDescription(format("block_%d.get%s()", field, type))
                        .append(getByteCodeExpresstion(context, offset, field)) //.getVariable("block_" + field)
